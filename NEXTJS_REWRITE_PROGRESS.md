...
Status: in-progress
Owner: agent/2026-07-16
Last updated: 2026-07-16
Proof: `npm run lint` ✓, `npm run typecheck` ✓, `npm run test` ✓ (390 unit), `npm run test:integration` ✓ for the touched features (analytics, known-users, known-groups, scheduled-tasks — real Postgres via Testcontainers). Analytics also verified live against the operator's dev server on 3200: `/analytics` renders (200) in dark mode with real data, all three numeric chart canvases painted, the period/chat/user filters drive a server re-query (verified `?granularity=month&chatId=…`), and the three JSON APIs return correct data. `npm run build` not run (`dont-clobber-running-dev-server` — the dev server is live on 3200).
