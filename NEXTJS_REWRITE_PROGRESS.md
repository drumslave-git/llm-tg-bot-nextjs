...
Next: **Priority 7 — Bot messaging: vision** (receive image/sticker/media and inject vision context into replies). The shared feature-1..6 gate is an operator-run live test with a real bot token + Tavily key + a dev-server restart (do not create credentials).
