...
- **Verified live against the real configured LLM** (`LLM_LIVE=1`, two gated integration suites, no Telegram needed — the dashboard-run path has no chat): (1) `tool-selection` 3/3 — the real model calls `browse_web` for a multi-step goal and one needing a click, carries the user's link into the goal, and correctly declines for a plain fact (it picks the lighter `read_web_page` for a single known-URL summary, exactly as the tool description tells it to — that overlap is by design). (2) `browse-live` 1/1 — the real model drove **real headless Chromium** to `https://example.com`, read the snapshot, and reported the *actual* page ("main heading is 'Example Domain'… for use in documentation examples"), grounded in the live page rather than invented. Both suites `describe.skipIf(!LLM_LIVE)`, so the default integration run stays token-free.
...
| Browser agent — generic toolbox only (priority 13, 2026-07-21) | done | user | **No scenario-specific tools in code** — *"we do not implement any special scenarios … just all tools which would allow llm to do a variety of tasks"*. The agent gets a generic browser toolset: navigate, back, click, type, scroll, read page (text + numbered interactive elements), read raw page source, screenshot, download, wait. The MVP's `browser_extract_media` video-ripping pipeline (autoplay triggering, embed-iframe hunting, HLS/ffmpeg muxing, media-URL network sniffing) is **not ported**; consequence accepted: "download this video" from a streaming player mostly won't work in v1 — the generic download tool fetches direct file URLs only. |
