...
Next: **Priority 6 — Visit/read link MCP tool** (fetch/read a URL with SSRF protections), built on the same MCP registrar pattern. The shared feature-1..5 gate is an operator-run live test with a real bot token + Tavily key (do not create credentials).
