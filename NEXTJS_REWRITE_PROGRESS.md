...
Next: **Priority 8 — Vision backfill background job** (describe the `pending` media rows in the background — `message_media.status='pending'` with bytes intact — with status/locking/idempotency; the `listPendingMedia` repository query already exists). Needs the **background job operating model** decision (Decision Notes, still `todo`). The shared feature-1..7 gate is an operator-run live test with a real bot token + a dev-server restart (do not create credentials).
