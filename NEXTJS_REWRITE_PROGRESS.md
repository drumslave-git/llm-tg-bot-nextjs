...
**Priority 11 — Analytics dashboard (done, 2026-07-15):** a rich stats dashboard at **`/analytics`** (ECharts). **Numeric metrics are aggregated live with SQL** (`date_trunc` + `GROUP BY` over `chat_messages` / `trace_events` / `known_users` / `message_media`) — no stored rollups, so they are exact and self-heal on edits/imports: message volume (human/bot), characters **processed vs generated**, active/new users, per-model **speed** (avg latency, tokens/s) + token volume, and deterministic **chat health** (👍/👎 satisfaction, error rate, reply latency → a composite score). Charts render at **hourly / daily / monthly / yearly / all-time** granularity with **global + per-chat + per-user** drill-down (URL-param filters → server re-query, the SSR-first pattern). **LLM-derived insight** (the only precomputed part) comes from a **nightly job** (`features/analytics/server/insights.ts`, the shared interval-scheduler + advisory-lock model): one LLM call per finished chat-day for **mood** (0–100) + **top topic** → `chat_day_insights` (self-healing on message-count change, like `chat_summary_days`), then a roll-up pass for **word of the period** + **top topic** + weighted mood into `period_insights` (month/year/all × global/per-chat). Fails **closed** — a garbled response leaves stored rows untouched. New feature module `features/analytics/*` (pure `period.ts`/`mood.ts`/`format.ts`/`types.ts`, server `repository.ts`/`metrics.ts`/`insights.ts`/`prompt.ts`/`scheduler.ts`), routes `GET /api/analytics/metrics|insights` + `GET/POST /api/analytics/insights/run`, UI `Chart.tsx` (client-only ECharts via `next/dynamic({ssr:false})`, validated per-mode palette + light/dark), `AnalyticsCharts`/`AnalyticsFilters`/`AnalyticsPanels`/`AnalyticsJobCard`. Live over the new `analytics` SSE topic; shared Debug at `/debug?feature=analytics-insights`; migration `0019`. Two SSR gotchas hit and fixed (see session log): the shared `JobStatusCard` must be driven from a **client** wrapper (never a Server Component with JSX `badges`/`notice` props), and ECharts must be kept off the server entirely. Tests: `period`/`mood`/`prompt` unit + `analytics.integration` (metrics aggregation, model-name merge, chat scope; insight job produces/rolls-up, idempotent, self-heals, fails-closed). **Not run (operator's call):** a real "Run now" (spends tokens on their data) and the boot-bound scheduler start (needs a dev restart — shows **Stopped** until then).
...
- 2026-07-15 (Priority 11): **Analytics dashboard (done)** — a rich ECharts stats
  ...
  - **Checks:** lint ✓, typecheck ✓, `npm run test` ✓ (374 unit),
...
| 11 | Analytics dashboard | done | defined (see 2026-07-15 log): live-SQL numeric metrics (volume/chars/users/model-speed/health) at hourly→all-time with per-chat/user drill-down + nightly LLM insight (mood, word of the period, top topic) | shared `/debug?feature=analytics-insights` (nightly insight runs, full per-day/period LLM bodies) | yes (shared `/api/traces/**/bundle`) | yes (`period`/`mood`/`prompt` pure unit; `analytics.integration` — real Postgres: metrics aggregation + model-name merge + chat scope; insight job produce/roll-up, idempotent, self-heal, fail-closed) | history (mirror + daily summaries), shared traces, LLM provider, background job model, shared Debug/SSE/job-card UI | Complete + verified live on 3200 (dark-mode render, 3 chart canvases, filter re-query, APIs). Operator gate: a real "Run now" spends tokens; the nightly scheduler is boot-bound (shows **Stopped** until a dev restart) |
...
| Analytics numeric metrics = live SQL (priority 11) | done | agent (flagged in plan, user-approved) | **Numeric metrics (message/char/token/latency volumes, users, models, health) are aggregated live with Postgres `date_trunc` + `GROUP BY` over the base tables, NOT stored rollups.** Exact and self-healing (a late edit/CSV import needs no rollup rebuild — the same problem `chat_summary_days` had to engineer around). Only the expensive LLM-derived insight (mood/word/topic), which can't be recomputed per page view, is precomputed by the nightly job into `chat_day_insights` (per chat-day) + `period_insights` (month/year/all roll-ups). Honors the user's "idle jobs similar to vision backfill" for exactly the metrics that need it. |
