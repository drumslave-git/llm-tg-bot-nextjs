...
**Browser agent — the download primitives (priority 13, 2026-07-21, from a live prod trace):** the operator sent "`<link>` download this video" and the bot **refused with "I'm a language model, not a video downloader" and called no tool** (trace `53225f26…`). Two fixes: (a) **selection** — the `browse_web` description now names and contradicts the model's false "I can't download" prior; reproduced 0/4 → **4/4** against the live LLM. (b) **capability, kept generic per the operator** (*"llm have to do everything on its own … provide tools like browser_get_network, browser_download_file, browser_download_stream"*): two new primitives — `browser_get_network` (the requests the page made, so the model finds the real `.mp4`/`.m3u8` a player fetched) and `browser_download_stream` (ffmpeg-mux an HLS/DASH manifest into an MP4), alongside the renamed `browser_download_file`. No "rip the video" scenario in code — the model composes. Pure HLS best-variant selection is unit-tested (+4 → **593 unit ✓**); both primitives **verified live against real endpoints** (`BROWSER_LIVE=1`): `browser_download_stream` muxed Mux's public HLS test stream into a real **466 MB playable MP4** via ffmpeg, and `browser_get_network` captured the page's real traffic (7 requests, 5 `.m3u8`). Ffmpeg is required for streams and present in dev; its absence is reported as a clear operator-fixable message. All live suites `describe.skipIf(!LLM_LIVE)` / `!BROWSER_LIVE`, so the default integration run stays token- and network-free. **Honest bound:** the individual primitives and the model's tool *selection* are proven on real endpoints; the model autonomously chaining `play → get_network → download_stream` on an arbitrary hostile tube site is not something I exercised against that class of site (it depends on the site's specific defenses + the model's reasoning, not on a missing capability).
...
| Browser agent — `browse_web` selection for downloads (priority 13, 2026-07-21, prod trace fix) | done | agent (from operator's live trace) | **The model would not call `browse_web` for "download this video/file `<link>`" — it refused with "I'm a language model, not a video downloader" and called no tool** (real trace `53225f26…`, and reproduced 0/4 on neutral URLs against the live LLM). Root cause: the model holds a strong "I can't download files" prior, and the old description's buried "or fetching a file" clause did not override it (the strong Igor persona made the refusal worse). Fix: the description now **asserts the capability and names the false prior directly** — "You CAN get a file for the user through this tool … DO NOT reply that you are 'just a language model' or 'cannot download files'", plus an explicit MUST-trigger on download/save/grab/fetch a file/video/image/document. Per `tools-self-describe-atomic` it lives entirely in the tool description. Verified live: 0/4 → **4/4** now select `browse_web`, and the "plain fact → no browse" negative case still holds. Regression pinned in the gated `tool-selection` suite. **The download-execution half was then resolved** (operator: keep it generic, add primitives not a scenario): `browser_get_network` + `browser_download_stream` were added so the model can find and fetch a streaming video itself — see the "generic toolbox only" row. |
