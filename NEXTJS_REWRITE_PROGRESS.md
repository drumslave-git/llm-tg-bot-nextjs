...
Proof: `npm run lint` ✓, `npm run typecheck` ✓, `npm run test` ✓ (355 unit), `npm run test:integration` ✓ for the touched features (known-users, known-groups, scheduled-tasks — real Postgres via Testcontainers), `npm run build` ✓ (the dev server on 3200 is the operator's own, unaffected). Per-chat language change also verified live in the browser against 3200 (Users DM-language column + Group detail Language card).
