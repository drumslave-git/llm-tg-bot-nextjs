...
| Vision recognition timing (priority 7 follow-up) | done | user | **Superseded 2026-07-14 (user):** media on an addressed message is **recognized before the reply** (inside `loadVision`, after the addressing/maintenance gates): describe → store the description on the `message_media` row + drop bytes → update history display → generate the reply. The reply receives **both the actual images/frames AND the recognition text** (user chose 2 vision passes over 1 text-only pass — higher fidelity, higher cost). Nothing is left to backfill for the addressed message; unaddressed media still uses the backfill job. History display reads `message_media.description` (no `chat_messages.content` mutation). Original decision (recognize *after* the delivered reply) applied until then. |
