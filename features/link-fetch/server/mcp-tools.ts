...
/**
 * Link reading exposed as an MCP tool. `read_page` reads ONE public web page with
 * headless Chromium and returns its readable text for the model to answer from.
 * The URL is SSRF-checked before the browser touches it, and the boundary never
 * throws — a blocked/failed read returns a clear error result rather than a broken
 * tool call. Registered under feature `link-fetch`, so every call is traced in its
 * own `mcp-tools-link-fetch` scope.
 */

export const READ_PAGE_TOOL = "read_page";

export const LINK_FETCH_TOOL_NAMES = [READ_PAGE_TOOL];
